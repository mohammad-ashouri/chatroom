    ...
    <div class="container mx-auto px-4 py-4">
        <div class="flex justify-between items-center">
            <div class="flex items-center gap-3">
                @if ($user !== null)
                    <figure
                        class="rounded h-10 w-10 flex-shrink-0 transition-opacity group-hover:opacity-90 {{ $chat->user2->profile }}">
                        <img src="{{ $chat->user2->profile }}" alt="{{ $chat->user2->name }}" class="rounded h-10 w-10"/>
                    </figure>
                    <p class="text-xl font-bold text-gray-800 dark:text-gray-100">{{ $chat->user2->name }}</p>
                @else
                    <p class="text-xl font-bold text-gray-800 dark:text-gray-100">
                        Please select private chat.
                    </p>
                @endif
            </div>
        </div>
